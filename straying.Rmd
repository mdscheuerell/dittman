...
Here is an example of fitting the multivariate model proposed in (\ref{SSM}) with 'MARSS'. I use some dummy data, but it would be trivial to substitute other data. 

```{r dummy_dat, eval=TRUE, fig.cap="Observations (points) of a negatively biased random walk (line."}
library(MASS)
...
yy <- xx + vv
## plot "data"
plot.ts(xx[1,], ylim=range(yy[1,]), ylab=expression(italic(x[t])~~or~~italic(y[t])))
points(yy[1,])
```
